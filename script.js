...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const zoomEffect = (entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                entry.target.classList.add('zoom'); // Add zoom class
            } else {
                entry.target.classList.remove('zoom'); // Remove zoom class
            }
        });
    };

    const observer = new IntersectionObserver(zoomEffect, options);

    services.forEach(service => {
        observer.observe(service); // Observe each service element
    });
});
// rotate text js code 
